...
vectors = []
for doc in docs:
    temp = es.termvectors(index="researchgate-data",  doc_type="paper", id=index_id,  term_statistics=False, field_statistics=False, fields=['text'])
    index_id += 1
    vec = temp["term_vectors"]["text"]["terms"]
    vec = dict(vec)
    vec_clean = {}
